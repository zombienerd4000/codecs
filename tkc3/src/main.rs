...
fn main() {
    ...
    if args.len() < 2 {
        eprintln!("tkc3: Transform-invariant LZ77 with deflate-style Huffman coding");
        eprintln!("Usage:");
        eprintln!("  tkc3 c <input> [output]   Compress");
        eprintln!("  tkc3 d <input> [output]   Decompress");
        eprintln!("  tkc3 t <input>            Test round-trip");
        eprintln!("  tkc3 bench                Run benchmarks");
        eprintln!("  tkc3 bench-corpus         Run Calgary/Canterbury corpus benchmarks");
        return;
    }
    ...
    match args[1].as_str() {
        ...
        "d" => {
            ...
            let elapsed = t0.elapsed();
            let dest = if args.len() > 3 { args[3].clone() }
                       else { args[2].trim_end_matches(".tkc3").to_string() };
            fs::write(&dest, &out).expect("write output");
...
fn run_bench() {
    ...
    // real file benchmarks (tkc3, gzip, zstd)
    println!("\n--- real files (tkc3) ---");
...
fn run_bench_corpus() {
    ...
    for (label, data) in &corpus {
        ...
        println!("{:<20} tkc3:{:>6}B ({:>5.1}%)  gzip:{:>6}B ({:>5.1}%)  diff:{}{}B",
