...
fn main() {
    ...
    if args.len() < 2 {
        eprintln!("tkc0: Transform-invariant LZ77 + rANS codec");
        eprintln!("Usage:");
        eprintln!("  tkc0 c <input> [output]   Compress");
        eprintln!("  tkc0 d <input> [output]   Decompress");
        eprintln!("  tkc0 t <input>            Test round-trip");
        eprintln!("  tkc0 bench                Run benchmarks");
        return;
    }
    ...
    match args[1].as_str() {
        ...
        "d" => {
            ...
            let elapsed = t0.elapsed();
            let dest = if args.len() > 3 { args[3].clone() }
                       else { args[2].trim_end_matches(".tkc").to_string() };
            fs::write(&dest, &out).expect("write output");
...
fn run_bench() {
    ...
    let codecs: [(&str, fn(&[u8]) -> Vec<u8>, fn(&[u8]) -> Vec<u8>); 4] = [
        ("--- tkc0 ---", compress, decompress),
        ("--- gzip -9 ---", gzip_enc, gzip_dec),
        ("--- zstd -19 ---", zstd_enc, zstd_dec),
        ("--- tlz3 (python) ---", tlz3_enc, tlz3_dec),
    ];
