...
fn main() {
    ...
    if args.len() < 2 {
        eprintln!("tkc2: Transform-invariant LZ77 with deflate-style Huffman coding");
        eprintln!("Usage:");
        eprintln!("  tkc2 c <input> [output]   Compress");
        eprintln!("  tkc2 d <input> [output]   Decompress");
        eprintln!("  tkc2 t <input>            Test round-trip");
        eprintln!("  tkc2 bench                Run benchmarks");
        eprintln!("  tkc2 bench-corpus         Run Calgary/Canterbury corpus benchmarks");
        return;
    }
    ...
    match args[1].as_str() {
        ...
        "d" => {
            ...
            let elapsed = t0.elapsed();
            let dest = if args.len() > 3 { args[3].clone() }
                       else { args[2].trim_end_matches(".tkc2").to_string() };
            fs::write(&dest, &out).expect("write output");
...
fn run_bench() {
    ...
    // real file benchmarks (tkc2, gzip, zstd)
    println!("\n--- real files (tkc2) ---");
